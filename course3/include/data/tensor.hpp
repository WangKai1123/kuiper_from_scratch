...
template <>
class Tensor<float> {  // 针对Tensor类的模板特化，处理float类型数据的特定实现
 public:
  ...
  Tensor(Tensor&& tensor) noexcept; // noexcept 这是一个异常规范，表示该构造函数保证不会抛出异常。
   ...
   * @return 张量的行数
   */
  uint32_t rows() const;  // 在成员函数后加上 const 表示该函数不会修改对象的任何成员变量。它承诺在函数执行过程中，类的状态保持不变。
